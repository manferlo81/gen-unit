...
describe('format with no options', () => {

  const format = createFormatter({})

  test('should return non finite numbers as string', () => {
  ...
  test('should format number', () => {
    ...
    positiveCases.forEach(({ value, expected }) => {
      expect(format(value)).toBe(expected)
      expect(format(-value)).toBe(value !== 0 ? `-${expected}` : '0')
    })
